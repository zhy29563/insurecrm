...
class _BackupRestorePageState extends State<BackupRestorePage> {
  ...
  Future<void> _loadSettings() async {
    ...
    setState(() {
      ...
      _intervalHours = [1, 6, 12, 24, 48, 72].contains(interval)
          ? interval
          : 24;
  ...
  Future<void> _refreshBackups() async {
    ...
    try {
      ...
      AppLogger.error('loading backups: $e');
    }
    if (!mounted) return;
  ...
  void _handleBackupAction(
    ...
    switch (action) {
      case 'share':
        try {
          final shareFile = await _backupService.exportBackupForSharing(
            info.path,
          );
        ...
        actions: [
          ...
          TextButton(
            ...
            onPressed: () async {
              ...
              await _backupService.deleteBackup(info.path);
              if (!ctx.mounted) return;
  ...
  Future<void> _createManualBackup(BuildContext ctx) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        ctx,
      ).showSnackBar(SnackBar(content: Text('备份成功：$backupPath')));
      _refreshBackups();
    } catch (e) {
      if (!ctx.mounted) return;
      ScaffoldMessenger.of(
                    ...
                    onPressed: () async {
                      ...
                      try {
                        ...
                        final backupPath = await backupService
                            .importBackupFromExternal(importFile);
                        if (!ctx.mounted) return;
                        _confirmRestore(
                          ctx,
                          backupPath,
