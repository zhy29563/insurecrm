...
class _ProductDetailPageState extends State<ProductDetailPage> {
  ...
  List<String> get _companyList {
    ...
    _cachedCompanyList =
        appState.products.map((p) => p.company).toSet().toList()..sort();
  ...
  Future<void> _loadAttachments() async {
    ...
    try {
      ...
      AppLogger.error('loading attachments: $e');
    }
  }
  ...
  Future<void> _pickImage() async {
    ...
    if (pickedFile != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '图片已添加 (${ImageUtils.formatFileSize(ImageUtils.getFileSize(compressedPath))})',
          ),
        ),
      );
    }
  ...
  Future<void> _pickVideo() async {
    ...
    if (pickedFile != null) {
      final savedPath = await ImageUtils.compressAndSave(
            ...
            ElevatedButton(
              onPressed: () {
                if (_newCompanyController.text.isNotEmpty) {
                  setState(
                    () => _companyController.text = _newCompanyController.text,
                  );
                  Navigator.pop(context);
                }
              },
  ...
  void _saveProduct() async {
    ...
    if (_formKey.currentState?.validate() ?? false) {
      ...
      final product = Product(
        ...
        sellingPoints: _sellingPointsController.text.isEmpty
            ? null
            : _sellingPointsController.text,
        category: _categoryController.text.isEmpty
            ? null
            : _categoryController.text,
        salesStartDate: _salesStartDateController.text.isEmpty
            ? null
            : _salesStartDateController.text,
        salesEndDate: _salesEndDateController.text.isEmpty
            ? null
            : _salesEndDateController.text,
        createdAt:
        ...
        try {
          for (int i = 0; i < _mediaFiles.length; i++) {
            ...
            try {
              ...
              AppLogger.error('saving attachment $i: $e');
            }
          }
        } catch (e) {
  ...
  Future<List<Customer>> _getProductCustomers(AppState appState) async {
    ...
    if (kIsWeb) {
      return appState.customers
          .where((c) => c.products.any((p) => p['id'] == widget.product!.id))
          .toList();
    } else {
      final db = DatabaseHelper.instance;
      final dbInstance = await db.database;
      final customerProductMaps = await dbInstance.rawQuery(
        'SELECT DISTINCT customer_id FROM customer_products WHERE product_id = ?',
        [widget.product!.id],
      );
      final customerIds = customerProductMaps
          .map((m) => (m['customer_id'] as num?)?.toInt() ?? -1)
          .toSet();
      return appState.customers
            ...
            child: Text(
              ...
              style: TextStyle(
                ...
                color: controller.text.isEmpty
                    ...
                    : (enabled ? _textPrimary : Colors.grey.shade500),
              ),
            ),
          ),
          if (enabled)
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              ...
              _buildDateSection(),
              _buildMediaSection(),
              if (isEditMode) ...[_buildCustomersSection(appState)],
              // 底部保存按钮
              if (_isEditing)
                Padding(
